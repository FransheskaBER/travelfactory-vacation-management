...
- [ ] 4.8 Frontend: auth flow — login page, persisted Pinia auth store, role guards via `meta.requiresRole`, full route skeleton with placeholder pages (paths + meta frozen in spec 4.8 §5), logout header button, expired-token handling (proactive + reactive), `dev_FRONT_END_URL` documented in backend/.env.example
- [ ] 4.9 Requester UI: request form, my-requests list with statuses, shared team-vacations view (D1); *replaces 4.8's `MyRequestsPage`/`TeamPage` placeholders — route paths + meta fixed by spec 4.8 §5*; *request form's `reason` field carries `maxlength="500"` — inherited from 4.7's cap (spec 4.7 §3)*; *shared components extracted in-chunk where 4.9 itself has two consumers or internal repetition: StatusBadge, RequestTable/List, FormField live in `src/components/` from the start (my-requests and team view must both consume RequestTable/List; the request form must consume FormField). PaginationControls explicitly OUT of scope — first consumer is 4.10*; *role-aware AppHeader nav links land here for both roles (spec 4.9 §8 Q4) — 4.10 inherits the header finished*
- [ ] 4.10 Validator UI: dashboard, filter by status + user, pagination, approve action, reject action with **required comment**; *replaces 4.8's `DashboardPage` placeholder — route path + meta fixed by spec 4.8 §5; consumes 4.9's StatusBadge + RequestTable/List + FormField as-is; adds PaginationControls to `src/components/`. If the dashboard needs a change to a shared component, extend via props — do not fork or copy it*; *resolve `userId`/`reviewedBy` to names client-side via `GET /users` — the dashboard serves bare uuids by design (spec 4.6 §8 Q10)*; *reject-comment field carries `maxlength="500"` — inherited from 4.7's cap (spec 4.7 §3)*
- [ ] 4.11 Reuse verification: grep for duplicated markup between the two interfaces; confirm StatusBadge, RequestTable/List, PaginationControls, FormField each have a single definition in `src/components/` with all consumers importing it. Fix any drift. No new extractions expected
