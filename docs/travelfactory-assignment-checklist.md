...
- [ ] 4.6 REST endpoints ported into `root.yaml` (route + request/response schemas, spec 4.2 §9) + handlers: auth, create request, list mine, list all (validator), approve, reject — pagination on list-all; `npm run codegen` after every YAML edit; **revert 4.2's temp guard on `/hello/:name`** (requireRole wrap in `handlers/hello.ts` + the 401/403 entries in root.yaml — see spec 4.2 §8 Q9); *end-to-end listener proof inherited from 4.5: approve via HTTP must produce the structured listener log line in server output (spec 4.5 §6 — the HTTP surface 4.5 lacks)*
