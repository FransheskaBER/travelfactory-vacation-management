...
- [ ] 4.7 Input validation + consistent error format (`{ error: { code, message } }` via `DomainError` → `toErrorResponse` — one shape everywhere); `reason` normalization on create (`""` → `null`, trimming) — spec 4.4 §3 defers it here; error-message wording also owned here (spec 4.4 §9 adjudication 1 — messages carry zero contract weight); *date validation (format + calendar) moved to 4.6's parser (spec 4.6 §8 Q7) — 4.7's spec must state that boundary*
...
- [ ] 4.10 Validator UI: dashboard, filter by status + user, pagination, approve action, reject action with **required comment**; *resolve `userId`/`reviewedBy` to names client-side via `GET /users` — the dashboard serves bare uuids by design (spec 4.6 §8 Q10)*
